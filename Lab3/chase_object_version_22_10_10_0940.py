...
class ChaseObject(Node):
    ...
    def __init__(self):
        ...
        timer_period = 1/3.75 # seconds = 1/f
    ...
    def _cmd_vel(self, new_position):
        ...
        # If there is no object, stop linear velocity 
        #and do a slow rotation counter to previous rotation direction
        if np.isnan(new_position.y):
            self.velocity.linear.x = 0.0
            #self.velocity.angular.z = self.velocity.angular.z /abs(self.velocity.angular.z )*-0.2
            return
        ...
        if distance_d <= 0:
            ...
            self.velocity.linear.x = self.distance_kp * error_distance + self.distance_ki*self.distance_integrator + self.distance_kd*distance_diff
            self.velocity.angular.z = self.theta_kp * error_theta + self.theta_ki*self.theta_integrator + self.theta_kd*theta_diff

        if abs(error_distance) < self.distance_envelope:
            self.distance_integrator /= 2
        
        if abs(error_theta) < self.theta_envelope:
            self.theta_integrator /= 2
